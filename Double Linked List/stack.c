...
void printInfoStack(Stack S) {
    ...
    if (isEmptyStack(S)) {
        ...
        int i = 1;
        while (P != NULL) {
            char tipeAktivitasStr[20], tipePriorityStr[20];
            switch (P->info.tipeAktivitas) {
                case 1: strcpy(tipeAktivitasStr, "Pinjam"); break;
                case 2: strcpy(tipeAktivitasStr, "Kembalikan"); break;
                case 3: strcpy(tipeAktivitasStr, "Batalkan"); break;
                default: strcpy(tipeAktivitasStr, "Unknown");
            }
            switch (P->info.tipePriority) {
                case DOSEN: strcpy(tipePriorityStr, "Dosen"); break;
                case MAHASISWA: strcpy(tipePriorityStr, "Mahasiswa"); break;
                case MASYARAKAT_UMUM: strcpy(tipePriorityStr, "Masyarakat Umum"); break;
                default: strcpy(tipePriorityStr, "Unknown");
            }
